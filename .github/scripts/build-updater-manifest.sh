...
#
# なぜビルドジョブではなくここで作るのか:
#   latest.json は3プラットフォーム共通の同名アセット。tauri-action は
#   「既存を読む → 自分の分をマージ → 上げ直す」で更新し、競合時は再試行するため
#   多くの場合は3つ揃う。ただし read-modify-write なので同時実行では
#   取りこぼす窓が残る。全ビルド完了後に1回だけ書けば競合自体が無くなる。
#
#   より重要なのは、欠けたときに気付けること。プラットフォーム別に書く方式では、
#   1つが失敗しても残りだけのマニフェストが添付されて公開まで進み得た。
#   ここで組み立てるなら、揃っていない時点でジョブごと落とせる。
#
# 必要な環境変数:
#   GH_REPO    - "owner/repo"
