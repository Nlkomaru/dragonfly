...
3. ワークフロー内で以下が順に走る。
   ...
   - `build` — Windows / macOS(aarch64) / Linux のバンドルと `latest.json` を
     ドラフトに添付する（3つ並列）
...
`latest.json` は3プラットフォーム共通の同名アセットだが、生成は `tauri-action` に任せている。
`tauri-action` は「既存を読む → 自分のプラットフォーム分をマージ → 上げ直す」形で更新するので、
3ジョブが並列に書いても3プラットフォーム分が揃う。

ビルドジョブは **3つ並列** で走る。
かつては `max-parallel: 1` で直列化しており、「並列だと片方の書き込みが失われる」と
説明されていたが、上記のとおりマージされるのでこの説明は誤りだった。
直列化は1リリースあたり約20分を余計に払っていただけなので外した。

Windows は `bundle.targets: "all"` により `.msi` と `-setup.exe`(NSIS) の両方に `.sig` が出る。
`tauri-action` の既定は `windows-x86_64` に `.msi` を割り当てるが、更新には NSIS を使いたいので
`updaterJsonPreferNsis: true` で明示的に切り替えている
（NSIS は Tauri が既定として推奨し、更新時のサイレントインストールが素直に通る。
`.msi` は再起動要求が絡みやすい）。

#### 既知の穴

マージ方式は read-modify-write なので、「読んだ後に他のジョブが書いた」という
取りこぼしは理論上あり得る。これは競合として検出されないため再試行も発火せず、
**ビルドが全部緑のまま1プラットフォーム欠けた `latest.json`** が公開され得る。
その OS の利用者にだけ更新が届かなくなり、しかも誰も気付けない。

実運用では観測されていない（姉妹リポジトリの `kodou` は `max-parallel` 無しの3並列で
14リリースを重ねて `latest.json` が揃っている）ため、検出用の仕組みを持つ対価に
見合わないと判断してこの穴は受け入れている。
更新が届かないという報告が出たら、まずこの可能性を疑うこと。

なお **ビルドが失敗した場合** は `publish` が `needs: build` なので公開されず、
リリースはドラフトのまま止まる（`v0.1.1` で macOS が一過性のネットワークエラーで
失敗したときも、そのように止まった）。失敗ジョブを再実行すれば復旧できる。
