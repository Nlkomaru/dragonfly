  ...
  return betterAuth({
    ...
    plugins: [
      ...
      apiKey({
        ...
        // apiKey プラグインの既定は「1鍵あたり 10 リクエスト / 24 時間」。
        // デスクトップは写真 1 枚につき 1 リクエスト送るので、11 枚目から必ず落ちる。
        // 用途に対して桁が違ううえ、鍵はサインイン許可リストを通った本人にしか出ないため、
        // この層でのレート制限は行わない（濫用対策は Cloudflare 側で行う）。
        //
        // 発行済みの鍵の行には rateLimitMax=10 が焼き込まれているが、
        // プラグイン側のこのフラグが行の値より先に評価されるので、鍵を作り直す必要はない。
