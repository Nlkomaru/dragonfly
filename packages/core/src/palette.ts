 ...
 * サイズ（apps/web の SAMPLE_MAX_EDGE、現在は長辺 192px）を変えると代表色がずれ、古いパレットとは
 * 距離を比べられなくなる。同じ version のパレット同士は必ず同じ条件で抽出されている、
 * という前提で buildDistanceMatrix が使われるため、そこを崩すなら必ずこの値を上げること。
 *
 * 3: SAMPLE_MAX_EDGE を長辺 64px から 192px に上げた。縮小しすぎで潰れていた
 *    細部の色が残るようになり、同じ写真でも 2 以前とは代表色が変わるため改版した。
 * 4: 代表色に加えて色ヒストグラム (histogram.ts) も保存するようにした。距離の計算は
 *    そちらで行うため、ヒストグラムを持たない 3 以前の行は距離を測れない。代表色そのものの
 *    抽出条件は 3 から変えていないが、再抽出させてヒストグラムを埋める必要があるため上げた。
 */
